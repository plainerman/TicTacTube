...
namespace TicTacTubeCore.Processors.Media
{
	...
	public abstract class BaseMediaInfoMerger<T> : IMediaInfoMerger<T> where T : IMediaInfo
	{
		...
		/// <summary>
		/// Merge multiple infos with the same <paramref name="greedy"/> parameter. Basically all infos are aggregated and pair-wise merged.
		/// </summary>
		...
		/// In other words, if <paramref name="greedy"/> is <c>true</c>, merge will return more data, but it may be completely wrong.</param>
		...
		/// <summary>
		/// Merge two data objects depending on whether it is a trusty source (<paramref name="trusted"/>) and if it should be <paramref name="greedy"/>.
		...
		/// <param name="b">The object that may override <paramref name="a"/>, if <paramref name="a"/> is <c>null</c> (and <paramref name="greedy"/> is <c>true</c>) or it is a <paramref name="trusted"/> source.</param>
		/// <param name="trusted">If the source is trusted, <paramref name="b"/> will override <paramref name="a"/> (as long as <paramref name="a"/> is <c>null</c>).</param>
		/// <param name="greedy">If <paramref name="greedy"/> is <c>true</c>, data that is <c>null</c> will be added to the new info, regardless of whether the other source is trustworthy or not.
		/// In other words, if <paramref name="greedy"/> is <c>true</c>, merge will return more data, but it may be completely wrong.</param>
		/// <returns>A merged media info containing a best guess which data is correct.</returns>
		...
		/// <summary>
		/// Merge two arrays depending on whether it is a trusty source (<paramref name="trusted"/>) and if it should be <paramref name="greedy"/>.
		/// Array <paramref name="a"/> or <paramref name="b"/> will be returned — or they may be merged.
		/// If both arrays are populated and the source is <paramref name="trusted"/> they will be merged. Otherwise it will be handled as in <see cref="MergeData{TOther}"/>, only that an empty array is handled like a <c>null</c> array.
